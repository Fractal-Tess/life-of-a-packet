...
<SingleQuestion
  client:load
  label="What information is typically found in a routing table?"
  choices={[
    'MAC addresses of local devices',
    'List of connected Wi-Fi networks',
    'Destination networks and next hop addresses',
    'Encryption keys for secure communication',
  ]}
  correct={2}
  explanation="Routing tables typically contain destination networks and next hop addresses, helping routers determine the best path for packets."
/>
...
<SingleQuestion
  client:load
  label="What does BGP stand for in networking?"
  choices={[
