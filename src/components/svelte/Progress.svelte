...
>
  ...
  {#each Array(driver.sectionsLength) as _, i}
    <button
      class="relative w-2 h-2 md:w-2 md:h-12 rounded-lg {i <=
      driver.ShownSections.length - 1
        ? 'bg-accent cursor-pointer'
        : 'bg-edge/30 cursor-not-allowed'}"
      onclick={() => driver.scrollToSection(i)}
      disabled={i > driver.ShownSections.length - 1}
      title={i <= driver.ShownSections.length - 1
        ? `Jump to section ${i + 1}`
        : 'Not available yet'}
      aria-label={i <= driver.ShownSections.length - 1
        ? `Jump to section ${i + 1}`
        : 'Not available yet'}
    >
      <div
        class="absolute inset-0 rounded-full transition-all duration-200 hover:scale-150 {i <=
        driver.ShownSections.length - 1
